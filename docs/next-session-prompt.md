...
- one NFC tag opens the app flow
...
- **one drink/tag** in v1 (UI does not expose any location/place field)
- one fixed product for the current tag
- one active batch for that drink
...
- `/` — signed-in ledger (balance cards dlužíš / dluží ti, current-drink card, tvé odběry) and a compact signed-out hero
...
- `/admin` — two panels: Pití a štítek (bootstrap wizard that creates product + hidden stock slot + tag in one submit if missing, otherwise drink name + NFC URL + re-mint + visible replace-drink form) and Lidé (members + invites)
...
- Keep the single drink/tag flow. Don't add place/location fields or multi-shelf UI patterns.
